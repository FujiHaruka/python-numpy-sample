...
def キャリブレートv2(元行列: ndarray, 移出ベクトル: ndarray, 小行列サイズ: int):
    ...
    移出ベクトルとの差の負成分を分配した行列 = 移出ベクトルとの差を行和に対する比率で分配する(
        移出ベクトルとの差が負になる行の元行列, 移出ベクトルとの差)
    列和に対する比率行列 = 列和に対する比率行列を求める(移出ベクトルとの差の負成分を分配した行列)
    #  列方向の調整量ベクトル
...
def 移出ベクトルとの差を行和に対する比率で分配する(移出ベクトルとの差が負になる行の元行列: ndarray, 移出ベクトルとの差: ndarray) -> ndarray:
    移出ベクトルとの差_縦 = 縦ベクトル化(移出ベクトルとの差)
    行和に対する比率行列 = 行和に対する比率行列を求める(移出ベクトルとの差が負になる行の元行列)
    return 行和に対する比率行列 * 移出ベクトルとの差_縦
